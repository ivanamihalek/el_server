	...
	// Search by ensembl ID, description or synonym
	    ...
	    // Not a valid ensembl id, so resolve it as a description or synonym
            List<Description> syn_descs = 
                Description.find("ensembl_gene_id in (from Resolution stable_id where synonym like ?)",
                "%"+query.toUpperCase()+"%").fetch();
	    ...
	    hits.addAll(descs);
	    render(hits); // Give the matching IDs for the user to choose, if any
	}
    }
